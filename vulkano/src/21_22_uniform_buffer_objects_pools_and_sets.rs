...
lazy_static! {
  static ref HELLO_INDEXED_RECT_VERTICES: Vec<My2dVertex> = vec![
    My2dVertex::new([-0.5f32, -0.5f32], [1.0f32, 0.0f32, 0.0f32]), // Top Left Red
    My2dVertex::new([0.5f32, -0.5f32], [0.0f32, 1.0f32, 0.0f32]), // Top Right Green
    My2dVertex::new([0.5f32, 0.5f32], [0.0f32, 0.0f32, 1.0f32]),  // Bottom Right Blue
    My2dVertex::new([-0.5f32, 0.5f32], [1.0f32, 1.0f32, 1.0f32]), // Bottom Left White
  ];
}
const HELLO_INDEXED_RECT_INDICES: [u16; 6] = [0, 1, 2, 2, 3, 0]; // {top left, top right, bottom right} and {bottom right, bottom left, top left}
...
pub struct HelloTriangleRenderer {
  ...
  index_buffer: Arc<dyn TypedBufferAccess<Content = [u16]> + Send + Sync>, /* The content of the
                                                                            * buffer is 16,
                                                                            * we're only sending
                                                                            * 6 not 65535
                                                                            * indices */
...
impl HelloTriangleRenderer {
  fn initialize(instance: &Arc<Instance>, window_surface: &HelloTriangleWindow) -> Self {
    ...
    //
    // It's likely both these buffers would be loaded from the asset file and
    // contained within one struct to be passed to the function that loads these
    // things to the GPU.
  ...
  fn create_graphics_pipeline(
    ...
    mod vertex_shader {
      vulkano_shaders::shader! {
        ty: "vertex",
        path: "../shaders/21_uniform_buffer_object.vert"
      }
    }
  ...
  fn create_vertex_buffer(graphics_queue: &Arc<Queue>) -> Arc<dyn BufferAccess + Send + Sync> {
    // Immutable buffer uses a memory mapped intermediate staging buffer interally,
    // builds a commandbuffer to copy to the final buffer, and submits that
    // command buffer as a future.
    let (buffer, future) = ImmutableBuffer::from_iter(
      HELLO_INDEXED_RECT_VERTICES.iter().cloned(),
  ...
  fn create_index_buffer(
    ...
    let (buffer, future) = ImmutableBuffer::from_iter(
      HELLO_INDEXED_RECT_INDICES.iter().cloned(),
